<template>
    <div class = "person">
        ...
        <hr>
        <h2>当前求和为{{ sum }}</h2>
        <button @click="changeSum">点我修改值</button>
    </div>
...
<script lang="ts" setup name = "Person"> // 相当于写了一个 setup 函数
    ...
    // 使用 reactive 不能够修改 对象
    function changeCar(){
        // 以下两种写法都不更新
        // car = {brand:'雅迪',price:30};
        // car = reactive({brand:'雅迪',price:30});

        // 能够使用 assign 来赋值
        // Object.assign(car,{brand:'雅迪',price:30});

        car.value = {brand:'雅迪',price:30};
    }
